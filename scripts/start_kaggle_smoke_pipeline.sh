...
log_line "start Kaggle smoke pipeline execute=$EXECUTE smoke_limit=$SMOKE_LIMIT"
...
if [[ "$EXECUTE" -eq 1 ]]; then
  ...
  if [[ "$ALLOW_EXECUTE_WITHOUT_STATUS_GATE" -eq 0 ]]; then
    status_args=(
      ...
      --dry-run-json "$DRY_RUN_REPORT_PATH"
...
if [[ "$EXECUTE" -eq 1 && "$NO_VALIDATION" -eq 0 ]]; then
  ...
  log_line "skip processed_smoke validation because --execute was not provided"
fi
...
log_line "finished Kaggle smoke pipeline execute=$EXECUTE"
