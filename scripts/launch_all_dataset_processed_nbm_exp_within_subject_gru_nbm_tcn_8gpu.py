#!/usr/bin/env python
"""Launch 120 within-subject GRU-BASE-NBM+TCN jobs across eight GPUs."""
...
from scripts.run_all_dataset_processed_nbm_exp_within_subject_gru_nbm_tcn import (
    EXPERIMENT_SCHEMA,
    FOLDS,
    NBM_VARIANT,
    SEEDS,
...
WORKER = REPO_ROOT / "scripts" / "run_all_dataset_processed_nbm_exp_within_subject_gru_nbm_tcn.py"
DEFAULT_EXPERIMENT = (
...
def ensure_plan(args: argparse.Namespace, seeds: tuple[int, ...]) -> dict[str, Any]:
    ...
    if path.is_file():
        ...
        if existing.get("plan_id") != proposed["plan_id"]:
            raise AssertionError("output-root has a different data/code/config identity; use a new --output-root")
        return existing
