#!/usr/bin/env python
"""Cross-platform launcher for the Kaggle FOG smoke pipeline.

The default mode is intentionally safe: it runs preflight, a streaming smoke
dry-run, and suite dry-run. Pass --execute to create processed_smoke and launch
the real smoke stages.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the Kaggle smoke pipeline; defaults to safe dry-run mode.",
    ...
    parser.add_argument("--execute", action="store_true", help="Create processed_smoke and run real smoke stages.")
    ...
    if args.smoke_limit < 1:
        parser.error("--smoke-limit must be >= 1")
...
def main() -> None:
    ...
    processed_smoke = kaggle_dir / "processed_smoke"
    log_path = (args.log_path or (repo / "outputs" / "logs" / "kaggle_smoke_pipeline.log")).resolve()
    preflight_json = (args.preflight_json or (repo / "outputs" / "kaggle_preflight_report.json")).resolve()
    dry_run_json = (args.dry_run_json or (repo / "outputs" / "kaggle_smoke_streaming_dry_run.json")).resolve()
    status_json = (args.status_json or (repo / "outputs" / "kaggle_status.json")).resolve()

    log_line(log_path, f"start Kaggle smoke pipeline execute={args.execute} smoke_limit={args.smoke_limit}")
    ...
    if not args.no_preflight:
        run_logged(
            [
                ...
                str(dataset_root),
                "--suite-config",
                str(repo / "configs" / "kaggle_smoke_suite.json"),
                "--smoke-limit",
    ...
    if args.execute and not args.no_validation:
        ...
        log_line(log_path, "skip processed_smoke validation because --execute was not provided")
    ...
    log_line(log_path, f"finished Kaggle smoke pipeline execute={args.execute}")
