#!/usr/bin/env python3
"""Run fresh composite-loss Conv-TCN NBMs followed by experiment C on 7 GPUs."""
...
NBM_WORKER = REPO_ROOT / "scripts" / "run_daphnet_conv_tcn_nbm_composite_fold.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--nbm-source-root", type=Path, default=None)
    parser.add_argument("--classifier-output-root", type=Path, default=None)
...
def validate_gpu_ids(value: str, check_hardware: bool) -> list[str]:
    ...
    if check_hardware:
        ...
        if any(int(gpu) >= count for gpu in gpu_ids):
            raise ValueError(
                f"requested GPUs {gpu_ids}, but nvidia-smi reports {count} GPUs"
            )
    return gpu_ids
...
def validate_frozen_nbms(args: argparse.Namespace, nbm_root: Path) -> list[dict[str, Any]]:
    ...
    for fold in FOLDS:
        ...
        if training["loss"] != EXPECTED_LOSS:
            raise AssertionError(f"fold {fold} composite-loss mismatch")
        expected_probabilities = (0.40, 0.40, 0.20)
        actual_probabilities = (
            float(augmentation["clean_probability"]),
            float(augmentation["gaussian_probability"]),
            float(augmentation["mask_probability"]),
        )
        if actual_probabilities != expected_probabilities:
            raise AssertionError(f"fold {fold} augmentation probabilities mismatch")
        if float(augmentation["gaussian_std"]) != args.gaussian_std:
            raise AssertionError(f"fold {fold} Gaussian std mismatch")
        if augmentation["assignment"] != (
            "mutually_exclusive_per_window_resampled_every_epoch"
        ):
            raise AssertionError(f"fold {fold} augmentation is not dynamic")
        if frozen.get("validation_mask") is not False:
            raise AssertionError(f"fold {fold} role-5 validation must be unaugmented")
        ...
        artifacts.append(
            {
                ...
                "best_validation_loss": float(done["best_validation_loss"]),
                "selection_metric": done["selection_metric"],
            }
        )
    return artifacts
...
def main() -> None:
    ...
    nbm_jobs = [
        {
            "id": f"fold{fold}_nbm_composite",
            "command": nbm_command(args, nbm_root, fold),
        }
        for fold in FOLDS
    ]
    job_specs = [(fold, seed) for fold in FOLDS for seed in seeds]
    train_jobs = [
        {
    ...
    seal = singleton_command(args, nbm_root, classifier_root, "seal")
    aggregate = singleton_command(args, nbm_root, classifier_root, "aggregate")
    plan = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "experiment": "conv_tcn_nbm_composite_dynamic_augmentation_then_C",
        "gpu_ids": gpu_ids,
        "nbm": {
            "jobs": 3,
            "maximum_concurrent_jobs": min(3, len(gpu_ids)),
            "max_epochs": args.nbm_max_epochs,
            "patience": args.nbm_patience,
            "learning_rate": args.nbm_learning_rate,
            "augmentation": "40% clean / 40% Gaussian(std=0.04) / 20% dynamic Mask",
            "loss": EXPECTED_LOSS,
        ...
        "classifier": {
            ...
            "maximum_concurrent_jobs": min(len(gpu_ids), len(train_jobs)),
        ...
        "strict_test_gate": (
            "all 9 group-C checkpoints and role-2/3 thresholds must be frozen "
            "before any role-0/1 access"
        ),
    }
    if args.dry_run:
