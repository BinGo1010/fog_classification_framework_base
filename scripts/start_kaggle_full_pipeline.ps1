...
$Processed = Join-Path $KaggleDir "processed"
if ($LogPath -eq "") {
    $LogPath = Join-Path $Repo "outputs\logs\kaggle_full_pipeline.log"
} else {
...
if ($DryRunReportPath -eq "") {
    $DryRunReportPath = Join-Path $Repo "outputs\kaggle_full_streaming_dry_run.json"
} else {
...
$SuiteConfig = "configs\kaggle_full_suite.json"
...
Write-LogLine "start Kaggle full pipeline execute=$Execute"
...
if ($Execute) {
    ...
    if (-not $AllowExecuteWithoutStatusGate) {
        $StatusArgs = @(
            ...
            "--full-dry-run-json", $DryRunReportPath,
...
if ($Execute -and (-not $NoValidation)) {
    ...
    Write-LogLine "skip processed validation because -Execute was not provided"
}
...
Write-LogLine "finished Kaggle full pipeline execute=$Execute"
