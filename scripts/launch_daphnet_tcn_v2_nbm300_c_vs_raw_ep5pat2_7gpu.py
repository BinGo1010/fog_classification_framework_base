#!/usr/bin/env python3
"""Seven-GPU launcher for Raw versus global-bottleneck TCN-v2 NBM scheme C."""
...
NBM_WORKER = REPO_ROOT / "scripts" / "run_daphnet_tcn_v2_nbm300_fold.py"
...
def validate_contract(args: argparse.Namespace) -> tuple[int, ...]:
    ...
    if args.nbm_max_epochs != 300 or args.nbm_patience != 20:
        raise ValueError("TCN-v2 NBM must use max_epoch=300 and patience=20")
    if args.nbm_dropout != 0.10:
        raise ValueError("this experiment freezes TCN-v2 dropout at 0.10")
    if args.tcn_max_epochs != 5 or args.tcn_patience != 2:
        raise ValueError("TCN classifier must use max_epoch=5 and patience=2")
    return nbm_seeds
...
def verify_existing_output_identity(root: Path, plan: dict) -> None:
    """Refuse to mix TCN-v2 with old Conv-TCN or another NBM backbone."""
    ...
    if experiment_config.exists():
        ...
        if frozen.get("nbm_kind") != "tcn_v2":
            raise RuntimeError("output-root already contains a non-TCN-v2 experiment")
...
def main() -> None:
    ...
    nbm_jobs = [
        {
            "id": f"fold{fold}_TCN_v2_NBM_seed{seed}",
    ...
    plan = {
        ...
        "nbm_kind": "tcn_v2",
        "strategy": (
            "7-GPU queue; 15 TCN-v2 NBMs then 30-classifier global test barrier"
        ),
        ...
        "nbm_backbone": (
            "multiscale TCN encoder->global z16->broadcast z+fixed Fourier16->"
            "TCN template decoder; no temporal skip/teacher forcing/input bypass"
        ),
        "nbm_parameter_count": 186_065,
        "bottleneck": "global [B,16]",
        "nbm_architecture_contract": {
    ...
    if args.phase in ("full", "nbm"):
        ...
        if args.phase == "nbm":
            print(f"TCN-v2 NBM COMPLETE output={root / 'nbm_source'}", flush=True)
            return
    if args.phase in ("full", "train"):
        for fold in FOLDS:
            for seed in seeds:
                ...
                if not done.exists():
                    raise FileNotFoundError(f"TCN-v2 NBM not frozen: {done}")
