#!/usr/bin/env python3
"""Run GRU-NBM scheme-C versus centered RAW on a seven-GPU job pool."""
...
def validate_contract(args: argparse.Namespace) -> tuple[int, ...]:
    ...
    if args.tcn_max_epochs != 10 or args.tcn_patience != 2:
        raise ValueError("TCN must use max_epoch=10 and patience=2")
    return nbm_seeds
...
def nbm_command(args: argparse.Namespace, fold: int, seed: int) -> list[str]:
    command = [
        ...
        "--fold", str(fold), "--seed", str(seed), "--device", "cuda",
        "--num-workers", str(args.num_workers),
        "--nbm-max-epochs", "300", "--nbm-patience", "20",
        "--nbm-hidden", "64", "--nbm-bottleneck", "16",
    ]
    if args.overwrite:
...
def pair_common(args: argparse.Namespace, source: Path) -> list[str]:
    values = [
        ...
        "--num-workers", str(args.num_workers),
        "--tcn-max-epochs", "10", "--tcn-patience", "2",
        "--required-nbm-max-epochs", "300", "--required-nbm-patience", "20",
    ]
    if args.overwrite:
...
def main() -> None:
    ...
    nbm_jobs = [{
        "id": f"fold{fold}_GRU_NBM_seed{seed}",
    ...
    plan = {
        "strategy": "7-GPU dynamic queue; 9 GRU-NBMs then 18-classifier global test barrier",
        "gpu_ids": gpu_ids,
        "folds": list(FOLDS), "methods": list(METHODS),
        "nbm_backbone": "GRU(9,64)->Linear(64,16)->Linear(16,64)->zero-input GRU(9,64)->Linear(64,9)",
        ...
        "classifier_training": "max10/pat2, weighted BCE, AdamW lr1e-3",
    ...
    if args.phase in ("full", "nbm"):
        ...
        if args.phase == "nbm":
            print(f"GRU-NBM COMPLETE output={root / 'nbm_source'}", flush=True)
            return
    if args.phase in ("full", "train"):
        for fold in FOLDS:
            for seed in seeds:
                done = root / "nbm_source" / f"seed_{seed}" / f"fold_{fold}" / "DONE_NBM.json"
                if not done.exists():
