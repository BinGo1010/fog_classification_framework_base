#!/usr/bin/env python
"""Launch the strict [r, delta(r)] GRU-NBM residual experiment on eight GPUs."""
...
def validate_source_root(
    source_root: Path,
    verify_initialization: bool = True,
) -> dict[str, Any]:
    ...
    if verify_initialization:
        for seed, hashes in hashes_by_seed.items():
            paired_r_delta_tcn(seed, next(iter(hashes)), torch.device("cpu"))
    return {
...
def build_plan(args: argparse.Namespace, seeds: tuple[int, ...]) -> dict[str, Any]:
    ...
    plan: dict[str, Any] = {
        ...
        "ablation": "remove abs(r); retain r and delta(r); reuse frozen Scaler/GRU-NBM/sigma",
        "raw_channels": 30,
        "tcn_input_channels": TCN_INPUT_CHANNELS,
        "paired_initialization_reference_channels": 90,
        "paired_reference_channel_selection": "0:30 for r; 60:90 for delta(r)",
        ...
        "event_metric": {
            ...
            "false_alarm": "role-0 only; same-record positive decisions <=1 s apart merged",
    ...
    if path.is_file():
        ...
        if existing.get("plan_id") != proposed["plan_id"]:
            raise AssertionError(
                "output-root has a different data/source/code/config identity; use a new root"
            )
        return existing
