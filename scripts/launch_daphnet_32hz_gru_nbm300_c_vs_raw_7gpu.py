#!/usr/bin/env python3
"""Seven-GPU launcher for 32-Hz GRU-NBM scheme-C versus centered RAW."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=REPO_ROOT
        / "outputs"
        / "daphnet_32Hz_gru_nbm300_C_vs_raw_tcn_ep20pat5_seedset_0_52_161_5216_52161",
    )
...
def validate_contract(args: argparse.Namespace) -> tuple[int, ...]:
    ...
    if args.tcn_max_epochs != 20 or args.tcn_patience != 5:
        raise ValueError("TCN must use max_epoch=20 and patience=5")
    return nbm_seeds
...
def main() -> None:
    ...
    nbm_jobs = [
        {
            "id": f"fold{fold}_GRU_NBM32_seed{seed}",
    ...
    plan = {
        ...
        "nbm_backbone": (
            ...
            "64-step zero-input GRU(9,64)->Linear(64,9)"
        ),
        ...
        "classifier_training": "max20/pat5, weighted BCE, AdamW lr1e-3",
        "full_input": "scheme C [r,abs(r),delta(r)] [B,27,64]",
        "raw_input": "role4 RobustScaler + per-window/per-axis centering [B,9,64]",
    ...
    if args.phase in ("full", "nbm"):
        ...
        if args.phase == "nbm":
            print(f"32-Hz GRU-NBM COMPLETE output={root / 'nbm_source'}", flush=True)
            return
