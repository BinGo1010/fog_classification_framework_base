#!/usr/bin/env python3
"""Seven-GPU launcher for Raw versus phase-conditioned GRU-v2 NBM scheme C."""
...
NBM_WORKER = REPO_ROOT / "scripts" / "run_daphnet_gru_v2_nbm300_fold.py"
...
def validate_contract(args: argparse.Namespace) -> tuple[int, ...]:
    ...
    if args.nbm_max_epochs != 300 or args.nbm_patience != 20:
        raise ValueError("GRU-v2 NBM must use max_epoch=300 and patience=20")
    if args.nbm_dropout != 0.10:
        raise ValueError("this experiment freezes GRU-v2 dropout at 0.10")
...
def verify_existing_output_identity(root: Path, plan: dict) -> None:
    """Refuse to mix GRU-v2 with an older GRU or another NBM backbone."""
    ...
    if experiment_config.exists():
        ...
        if frozen.get("nbm_kind") != "gru_v2":
            raise RuntimeError("output-root already contains a non-GRU-v2 experiment")
...
def main() -> None:
    ...
    nbm_jobs = [
        {
            "id": f"fold{fold}_GRU_v2_NBM_seed{seed}",
    ...
    plan = {
        ...
        "nbm_kind": "gru_v2",
        "strategy": (
            "7-GPU queue; 15 GRU-v2 NBMs then 30-classifier global test barrier"
        ),
        ...
        "nbm_backbone": (
            "BiGRU(9,96x2dir)->global z16->decoder h0; fixed Fourier16->"
            "2-layer GRU96->MLP96x48x9; no skip/teacher forcing"
        ),
        "nbm_parameter_count": 172_697,
    ...
    if args.phase in ("full", "nbm"):
        ...
        if args.phase == "nbm":
            print(f"GRU-v2 NBM COMPLETE output={root / 'nbm_source'}", flush=True)
            return
    if args.phase in ("full", "train"):
        for fold in FOLDS:
            for seed in seeds:
                ...
                if not done.exists():
                    raise FileNotFoundError(f"GRU-v2 NBM not frozen: {done}")
