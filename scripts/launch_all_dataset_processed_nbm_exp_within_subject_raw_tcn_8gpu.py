#!/usr/bin/env python
"""Launch the 8-subject x 3-fold x 5-seed RAW+TCN experiment on 8 GPUs."""
...
from scripts.run_all_dataset_processed_nbm_exp_within_subject_raw_tcn import (
    EXPERIMENT_SCHEMA,
    FOLDS,
    SEEDS,
    SUBJECTS,
...
WORKER = REPO_ROOT / "scripts" / "run_all_dataset_processed_nbm_exp_within_subject_raw_tcn.py"
CRITICAL_CODE = (
    WORKER,
...
DEFAULT_EXPERIMENT = "all_dataset_processed_NBM_Exp_within_subject_raw_tcn_ep5pat2_seedset_0_52_161_5216_52161"
...
def build_plan(args: argparse.Namespace, seeds: tuple[int, ...]) -> dict[str, Any]:
    ...
    plan: dict[str, Any] = {
        ...
        "folds": list(FOLDS),
        "seeds": list(seeds),
        "train_job_count": len(SUBJECTS) * len(FOLDS) * len(seeds),
        "evaluate_job_count": len(SUBJECTS) * len(FOLDS) * len(seeds),
        "sampling_rate_hz": 64,
        "window_samples": 128,
        "stride_samples": 64,
        "input_channels": 30,
        "representation": "RAW RobustScaler(role4) + per-window/per-axis centering",
        "batch_size": int(args.batch_size),
        "tcn_max_epochs": int(args.tcn_max_epochs),
        "tcn_patience": int(args.tcn_patience),
        "event_metric": "coverage_aware.v2; 2 positive windows; merge gap 0.5 s; FA/h on evaluated valid Non-FoG union coverage",
    }
    ...
    if path.is_file():
        ...
        if existing.get("plan_id") != proposed["plan_id"]:
            raise AssertionError(
                "output-root belongs to a different data/code/config identity; use a new --output-root"
            )
        return existing
...
def main() -> None:
    ...
    if seeds != SEEDS:
        raise ValueError(f"this experiment freezes the original five seeds: {SEEDS}; received {seeds}")
    ...
    if args.dry_run:
        print("DRY RUN: no model training, sealing, evaluation, or aggregation executed", flush=True)
    ...
    if args.phase in ("full", "evaluate"):
        if not (args.output_root / "TRAINING_BARRIER.json").is_file():
            raise FileNotFoundError("evaluate phase requires TRAINING_BARRIER.json; run --phase train first")
