...
from scripts import launch_daphnet_64hz_raw_tcn_ep256pat5_5seed_7gpu as launcher
...
def test_fixed_ep256pat5_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    args = default_args(monkeypatch)
    assert args.output_root.name == (
        "daphnet_64Hz_raw_tcn_ep256pat5_seedset_0_52_161_5216_52161"
    )
    assert args.tcn_max_epochs == 256
    assert args.tcn_patience == 5
    assert launcher.base.validate_contract(args) == (0, 52, 161, 5216, 52161)
    assert launcher.base.os.environ["MKL_THREADING_LAYER"] == "GNU"
    assert "MKL_SERVICE_FORCE_INTEL" not in launcher.base.os.environ


def test_raw_only_job_grid_and_worker_command(
    ...
    assert option_value(command, "--tcn-max-epochs") == "256"
    assert option_value(command, "--tcn-patience") == "5"


def test_budget_drift_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    args = default_args(monkeypatch)
    args.tcn_max_epochs = 255
    with pytest.raises(ValueError, match="max_epoch=256"):
        launcher.base.validate_contract(args)


def test_dry_run_is_15_raw_jobs_without_nbm(
    monkeypatch: pytest.MonkeyPatch,
    ...
    assert '"maximum_epochs": 256' in output
