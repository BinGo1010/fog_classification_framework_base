...
from scripts import (
    launch_daphnet_64hz_raw_tcn_batch64_ep5pat2_5seed_7gpu as launcher,
)
...
def test_fixed_batch64_ep5pat2_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    args = default_args(monkeypatch)
    assert args.output_root.name == (
        "daphnet_64Hz_raw_tcn_batch64_ep5pat2_"
...
def test_classifier_loader_really_batches_64_samples() -> None:
    x = np.zeros((130, 128, 9), dtype=np.float32)
    y = np.zeros(130, dtype=np.float32)
    loader = classifier_loader(
        x, y, shuffle=False, seed=0, num_workers=0, batch_size=64
    )
...
def test_batch_or_budget_drift_is_rejected(
    ...
    args = default_args(monkeypatch)
    args.tcn_max_epochs = 64
    with pytest.raises(ValueError, match="max_epoch=5"):
        launcher.base.validate_contract(args)


def test_dry_run_records_batch64_without_nbm(
    monkeypatch: pytest.MonkeyPatch,
